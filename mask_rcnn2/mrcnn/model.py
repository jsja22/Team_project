...
def resnet_graph(input_image, architecture, stage5=False, train_bn=True):
    ...
    x = KL.Activation('relu')(x)
    C1 = x = KL.MaxPooling2D((3, 3), strides=(2, 2), padding="same")(x)    #피라미드 아래에서 위로 올라가는 과정 
    # Stage 2
...
class PyramidROIAlign(KE.Layer):
    ...
    def call(self, inputs):
        ...
        #inputs([(1,None,4),(None,14)],feature_maps)
        
        # Feature Maps. List of feature maps from different level of the
        # feature pyramid. Each is [batch, height, width, channels]
        feature_maps = inputs[2:]
        #print("feature_maps:",feature_maps) #fpn_p2~p5까지 4개의 (None,None,None,256) feature maps
        ...
        for i, level in enumerate(range(2, 6)):
            ...
            #bilinear interpolation => RoIAlign 피쳐맵 위에 있는 그리드 포인트로부터 샘플링 포인트점을 알아내기위한 방법
            #결국 ROI에서 얻고자하는건 그 샘플링 포인트 
        ...
        ####pooled#########
        # Tensor("roi_align_classifier/Reshape:0", shape=(1, None, 7, 7, 256), dtype=float32)
        # Tensor("roi_align_mask/Reshape:0", shape=(1, None, 14, 14, 256), dtype=float32)
        return pooled
...
def rpn_graph(feature_map, anchors_per_location, anchor_stride):
    ...
    # 엥커에 대한 확률 => BG/FG 각각의 SCORE가 두개로 나옴 
    ...
    print("#####################")
    print(rpn_class_logits) #(None, None, 2)  (이미지수, 한이미지에서 찾을수있는 Roi수 , (True,False))
    print(rpn_probs)        #(None, None, 2)  (이미지수, 한이미지에서 찾을수있는 Roi수 , (BG/FG) SCORE)
    print(rpn_bbox)         #(None, None, 4)  (이미지수, 한이미지에서 찾을수있는 Roi수 , (x1,y1,x2,y2))
    print("#####################")
